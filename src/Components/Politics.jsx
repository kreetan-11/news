...
export default function Politics() {
    ...
    function getPolitics(){
        axios.get(`https://newsapi.org/v2/everything?q=Politics&apiKey=e3fd36ba7f564e6897a2ae73c4beaa4d`)
        .then(function (response) 
        {
            // console.log(response.data.articles)
            setPolitics(response.data.articles)
    })
    };
  return (
   < >
 <div className="py-16 bg-gradient-to-br from-green-50 to-cyan-100">  
  <div className="container m-auto px-6 text-gray-600 md:px-12 xl:px-6">
    <div className="mb-12 space-y-2 text-center">
      ...
      <h2 className="text-2xl text-cyan-900 font-bold md:text-4xl">Political-news</h2>
    ...
    <div className="grid gap-12 lg:grid-cols-2">
        ...
        <div className="sm:w-7/12 pl-0 p-5">
          <div className="space-y-2">
            ...
            <Link to={`/Detail/${q.source.id}`} className="block w-max text-cyan-600"> Read more </Link>
          </div>
        </div>
      </div>
        ))}
